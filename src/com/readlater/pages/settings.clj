...
(defn settings-page [{:keys [biff/db]}]
  (let [settings (db/app-settings db)
    ...
    (ui/page (merge (db/base-page-opts db) {:active :settings :title "Settings" :crumbs "Settings"})
             [:div {:class "px-4 sm:px-6 lg:px-10 py-6 sm:py-8 max-w-3xl mx-auto w-full"}
              ...
              [:section {:class "mt-8 pb-8 border-b border-stone-200"}
               [:h2 {:class "text-xl font-semibold mb-1"} "Discover preferences"]
               [:p {:class "text-sm text-stone-500 mb-4"}
                "Укажи темы или ключевые слова — Claude будет учитывать их при формировании подборок Discover."]
               [:form {:hx-post    "/api/settings/interests"
                       :hx-swap    "none"
                       :hx-on--after-request "if(event.detail.successful){showSavedToast()}"}
                [:textarea {:name        "interests"
                            :rows        3
                            :placeholder "Например: Rust, machine learning, distributed systems, WebAssembly"
                            :class       "textarea textarea-bordered w-full bg-stone-50 text-sm resize-none focus:outline-none focus:border-accent-400 mb-3"
                            :onkeydown   "if(event.key==='Enter'&&!event.shiftKey){event.preventDefault();this.form.requestSubmit()}"}
                 interests]
                [:button {:type "submit" :class "btn btn-sm border-none text-white gap-1.5" :style {:background "#8B5A3C"}}
                 [:i {:data-lucide "save" :class "icon-sm"}]
                 "Save preferences"]]]
              [:section {:class "mt-8"}
...
(defn save-interests [{:keys [biff/db params] :as ctx}]
  (let [interests  (str/trim (or (get params :interests) (get params "interests") ""))
        existing   (or (db/app-settings db) {})
        updated    (merge (dissoc existing :db/doc-type)
                          {:db/doc-type        :settings
                           :xt/id              :settings/global
                           :settings/interests (when (seq interests) interests)})]
    (biff/submit-tx ctx [updated])
    {:status 200 :body {:ok true}}))

(def routes    [["/settings" {:get #'settings-page}]])
(def api-routes [["/api/settings/interests" {:post #'save-interests}]])
