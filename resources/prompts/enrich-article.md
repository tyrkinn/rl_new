...
1. Прочитай заголовок, подзаголовок, первый и последний абзац.
2. Просмотри заголовки секций (h2/h3) и первые предложения каждой.
3. Полное чтение только если статья короткая (<1500 слов) или ты не уверен в tldr/keywords.
...
Если страница недоступна (paywall, 404, требует логин) — верни:
{"error": "краткое описание", "reason": "paywall|notfound|login_required|other"}

Если это не статья/эссе, а инструмент/сервис/лендинг — верни kind:"bookmark" вместо "article".
